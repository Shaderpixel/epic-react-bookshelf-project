...
// we don't want to accept the user here anymore because the function can't access context since it's not a hook and its being called as a cleanup function of a useEffect. However we can wrap that function in a custom hook that has the user in it:
// Instead we'll make a new hook that gets the user and then returns this function
...
function useRefetchBookSearchQuery() {
  const client = useClient()
  return React.useCallback( // The memoized version of ths callback
   async () => {
      queryCache.removeQueries('bookSearch')
